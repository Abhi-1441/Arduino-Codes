...
const byte pin_LED = 9; // LED that turns on when metal is detected
char x ='1'; 
void metal_detection_func();
void setup() {
...
void loop() {
  if(x=='a'){
    metal_detection_func();
  }
  if(Serial.available()){
    x=Serial.read();
    if(x=='a'){
      Serial.println("Metal Detection Started");
    }
    else{
      Serial.println("Metal Detection Stopped");
      digitalWrite(pin_LED, LOW);
